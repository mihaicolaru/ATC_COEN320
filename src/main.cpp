...
int main() {
  ...
  void *ptr =
      mmap(0, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, shm_planeList, 0);
  ...
  //	std::shared_ptr<std::vector<Plane*>> sharedPlanes =
  // std::make_shared<std::vector<Plane*>> (std::move(planes));
  //
  PSR psr(planes.size());
  psr.start();
